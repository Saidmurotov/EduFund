...
function FilterPanel({ filters, setFilters, toggleFilter, clearFilters, _onClose }) {
  return (
    <div className="space-y-8">
      ...
      {/* Funding Type */}
      <div>
        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 block">Moliyalashtirish</label>
        <div className="flex flex-wrap gap-2">
          {FUNDING_TYPES.map(f => (
            <Chip
              key={f.value}
      ...
      <button
        ...
        className="w-full py-2.5 rounded-xl border border-[#334155] text-xs font-bold text-slate-400 hover:text-white hover:border-slate-500 transition-colors uppercase tracking-widest"
      >
        Tozalash
      </button>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className={[
            "lg:hidden px-4 rounded-xl border flex items-center gap-2 transition-all",
      ...
      {isFilterOpen && (
        <div className="lg:hidden bg-[#1E293B] border border-[#334155] rounded-2xl p-6 shadow-2xl animate-[slideDown_0.2s_ease-out] max-h-[70vh] overflow-y-auto custom-scrollbar">
      ...
      {/* Desktop Layout: sidebar filter + results */}
      <div className="flex gap-6">
        {/* Desktop Filter Sidebar — always visible */}
        <aside className="hidden lg:block w-72 shrink-0">
          <div className="sticky top-8 bg-[#1E293B] border border-[#334155] rounded-2xl p-6 max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar">
            <h3 className="text-lg font-bold text-slate-50 mb-6">Filterlar</h3>
            <FilterPanel
              filters={filters}
              setFilters={setFilters}
              toggleFilter={toggleFilter}
